...
const RegressionChart: React.FC<RegressionChartProps> = ({ data, regression, userSqFt }) => {
  if (!data || data.length === 0) {
    return (
      <div className="w-full h-[400px] bg-white rounded-2xl p-4 border border-slate-100 shadow-sm flex items-center justify-center text-slate-500">
        No comparable data available to display the regression chart.
      </div>
    );
  }
